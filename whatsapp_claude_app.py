...
def get_recipients(group="directors"):
    try:
        creds_file = open(GOOGLE_CREDS_FILE)
        ...
        if group == "all":
            ...
            recipients = [r for r in rows if str(r.get("group", "")).lower() == group.lower() and str(r.get("active", "")).upper() == "YES"]
...
def notify():
    ...
    return jsonify({"status": "success", "event": event, "group": group, "sent_to": len(results), "results": results}), 200


def build_event_message(event, data):
    messages = {
        "po_approved": f"*PO Approval Alert*\n\nPO Number: {data.get('po_number','N/A')}\nAmount: Rs.{data.get('amount','N/A')}\nApproved by: {data.get('approved_by','N/A')}\nDate: {data.get('date','Today')}\n\nPlease review and take necessary action.",
        "invoice_generated": f"*Invoice Generated*\n\nInvoice No: {data.get('invoice_no','N/A')}\nCustomer: {data.get('customer','N/A')}\nAmount: Rs.{data.get('amount','N/A')}\nDue Date: {data.get('due_date','N/A')}",
        "low_stock": f"*Low Stock Alert*\n\nItem: {data.get('item','N/A')}\nCurrent Stock: {data.get('stock','N/A')}\nReorder Level: {data.get('reorder_level','N/A')}\nPlease initiate purchase order.",
...
def send_whatsapp_message(to_number, message_text):
    ...
    print(f"Sent to {to_number}: {response.status_code} - {response.json()}")
